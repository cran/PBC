...
double C_miwa(int steps, double* dcorr, double* dupper, double* dlower, int* infinvalue) {
...
*/
    ...
    double output,*f, r[MAXM][MAXM][MAXM], hv[MAXM][MAXM], d[MAXM][MAXM];
    //int *infinvalue;
    struct GRID   grid;
...
*/   
 ...
 }else if (checkall(infinvalue,infinlength,0)==1 )
 {
  ...
  answer = orthant(dim, r, hv, &ncone, &grid);
...
/*
 * 
 *  Add function to calculate 2-variables copula (by Pham Van Trung)
 *
 */
...
double calcul(double u1, double n1, double u2, double n2, double a){
  ...
  //dcorr[2] = REAL(a)[0];
  dcorr[3] = 1.0;
  double* dupper = new double[2];
  dupper[0] =  Rf_qnorm5(pow(u1,(double)1/n1), 0.0, 1.0, 1, 0);
  dupper[1] =  Rf_qnorm5(pow(u2,(double)1/n2), 0.0, 1.0, 1, 0);
  //dupper[0] =  Rf_qnorm5(pow(REAL(u1)[0], 1/REAL(n1)[0]), 0.0, 1.0, 1, 0);
  //dupper[1] =  Rf_qnorm5(pow(REAL(u2)[0], 1/REAL(n2)[0]), 0.0, 1.0, 1, 0);
  double inf = numeric_limits<double>::infinity();
  double* dlower = new double[2];
  dlower[0] = -inf;
  dlower[1] = -inf;
  int* infinvalue = new int[2];
  infinvalue[0] = 2;
  infinvalue[1] = 2;
  res = C_miwa(steps, dcorr, dupper, dlower, infinvalue);
  //SEXP ret = wrap(res);
  return res;
}
